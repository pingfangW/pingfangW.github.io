...
![]({{ site.url }}/assets/ML/linearRegression_04.jpg){:height="30%" width="30%"}
...
X = np.insert(X, 0 , values=1, axis=0)  # 添加截距项
X = np.mat(X)
Y = np.mat(Y).T  #为了矩阵维度与之前设定一直
...
class logisticRegression():
	...
	def LR_dev(self):
		Y_predict = sigmoid()
		return -1./n (self.Y - Y_predict).T * self.X

	def gradientDescent(eta, num_iters):
		L = np.zeros((num_iters, 1))  # 计算每次迭代的损失值
