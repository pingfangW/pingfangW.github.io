...
![]({{ site.url }}/assets/ML/linearRegression_03.jpg){:height="30%" width="30%"}
...
$$-\sum_{k=1}^K[y=k]log\frac{e^{z_k}}{\sum_{j=1}^k e^{z_j}}$$
...
令$$\hat{y_i} = \pi(x_i) = P(y_i=1 \mid x_i,w)$$，则 $$1 - \pi(x_i) = 1-P(y_i=1|x_i,w) = P(y_i=0 \mid x_i,w)$$
...
def LR_dev(Y, Y_predict, X):
	n = Y.shape[0]
	return -1./n (Y - Y_predict).T * X

def gradientDescent(X, Y, Y_predict, w, eta, num_iters):
	n = Y.shape[0]  # 样本量
	d = w.shape[0]  # 参数量
...
$$\prod_{i=1}^N [\pi(x_i)^{y_i} [1 - \pi(x_i)]^{1-_i}]$$

其中，$$\pi(x) = P(Y=1|x)$$，$$1 - \pi(x) = 1 - P(Y=1|x) = P(Y=0|x)$$
